...
return {
  ...
  image:wrk.properties.Image.files[0].file.url,
};
}
...
const getRatesData = (data) => {
  return {
    ...
    dollars:data.properties['Price (USD)'].number,
    description:data.properties['Work Description'].rich_text[0].text.content,
  };
  }
  ...
  const getAddtlFeesData = (data) => {
    return {
      id: data.id,
      name:data.properties.Name.title[0].plain_text,
      pesos:data.properties.Price.rich_text[0].text.content,
...
export const getWillDraw = async () => {
  const response = await notion.pages.retrieve({ 
    page_id: process.env.NOTION_COM_WILLDRAW,
  });
...
export const getWontDraw = async () => {
  const response = await notion.pages.retrieve({ 
    page_id: process.env.NOTION_COM_WONTDRAW,
  });
...
export const getTOC = async () => {
  const response = await notion.pages.retrieve({ 
    page_id: process.env.NOTION_COM_TOC,
  });
